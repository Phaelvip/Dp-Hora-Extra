...
class VendaRepository:
    ...
    def CadastrodeDadosdaWoodpaper(self, data, valor): 
       # SELECT  round(SUM(valor),2) AS VALOR_FINAL as VALOR_FINAL FROM  `'+db.getDbName()+'`.vendas where boleta = 0 AND NFISCAL >=1  AND NATOPERDS='VENDA' AND DATA >= '2022-07-06' AND DATA <= '2022-07-07';
        ...
        try:
            ...
            cursor.execute('insert into `'+db.getDbName()+'`.woodpaper (data, valor) values (%s,%s)', valores)
            con.commit()
            db.closeConn(cursor)    
            return True
        except Error as e :
    ...
    def BuscarDadosdaWoodpaper(self, anoInicial, anoFinal): 
       # SELECT  round(SUM(valor),2) AS VALOR_FINAL as VALOR_FINAL FROM  `'+db.getDbName()+'`.vendas where boleta = 0 AND NFISCAL >=1  AND NATOPERDS='VENDA' AND DATA >= '2022-07-06' AND DATA <= '2022-07-07';
         #con = mysql.connector.connect(host='127.0.0.1', user='root', database=' `'+db.getDbName()+'`',password='q9w8e7#MTB')
        db = DbConnection()
        ...
        try:
            con = db.getConn()
            cursor = con.cursor(dictionary = True)
            valores = (anoInicial, anoFinal)
            #cursor.execute('SELECT nome, senha FROM ` `'+db.getDbName()+'``.senha WHERE nome = %s AND senha = %s', valores)
            cursor.execute('SELECT  round(valor,2) as VALOR_FINAL FROM `'+db.getDbName()+'`.woodpaper where data >=%s and data <=%s;', valores)
            ver = cursor.fetchone()
            db.closeConn(cursor)    
            return ver
        except Error as e :
    ...
    def valorDeDepreciacao(self, valor): 
       # SELECT  round(SUM(valor),2) AS VALOR_FINAL as VALOR_FINAL FROM  `'+db.getDbName()+'`.vendas where boleta = 0 AND NFISCAL >=1  AND NATOPERDS='VENDA' AND DATA >= '2022-07-06' AND DATA <= '2022-07-07';
        ...
        try:
            ...
            cursor = con.cursor()
            #cursor.execute('SELECT nome, senha FROM ` `'+db.getDbName()+'``.senha WHERE nome = %s AND senha = %s', valores)  
            cursor.execute("delete from `"+db.getDbName()+"`.depreciacao") 
            cursor.execute("truncate table `"+db.getDbName()+"`.depreciacao")             
            insertString = "insert into `"+db.getDbName()+"`.depreciacao (valor) values ( {} )".format(float(valor))
    ...
    def BuscarDadosDepreciacao(self): 
       # SELECT  round(SUM(valor),2) AS VALOR_FINAL as VALOR_FINAL FROM  `'+db.getDbName()+'`.vendas where boleta = 0 AND NFISCAL >=1  AND NATOPERDS='VENDA' AND DATA >= '2022-07-06' AND DATA <= '2022-07-07';
        ...
        try:
            ...
            cursor = con.cursor(dictionary = True)
            #cursor.execute('SELECT nome, senha FROM ` `'+db.getDbName()+'``.senha WHERE nome = %s AND senha = %s', valores)
            cursor.execute('SELECT  round(SUM(valor),2) as VALOR_FINAL FROM `'+db.getDbName()+'`.depreciacao;')
            ver = cursor.fetchone()
            db.closeConn(cursor)    
            return ver
        except Error as e :
